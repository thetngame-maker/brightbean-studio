...
from .ugc_discovery_tasks import enqueue_saved_discovery_search
    ...
    return JsonResponse(
        {
            ...
            "queued_count": sum(1 for i in searches if i.get("last_run_status") == "queued"),
            "running_count": sum(1 for i in searches if i.get("last_run_status") == "running"),
        }
    )
...
def queue_background_test_run(request, workspace_id, search_id):
    ...
    if not saved_search:
        ...
        saved_search.get("platform") == "instagram"
        and saved_search.get("search_type") == "location"
        and not saved_search.get("resolved_location_url")
    ):
        return redirect("ugc:location_candidates", workspace_id=workspace.id, search_id=search_id)
    elif not live_provider_ready():
        messages.error(request, "Connect a live discovery provider before running this search.")
    elif enqueue_saved_discovery_search(str(workspace.id), str(search_id), force=True):
        messages.success(request, "Live discovery queued. Previous results are preserved until this run finishes.")
    else:
        messages.info(request, "This discovery search is already queued or running.")
