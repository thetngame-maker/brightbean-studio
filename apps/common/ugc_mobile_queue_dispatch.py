...
def moderation_queue(request, workspace_id):
    """Give Approved its own Ready/Drafted mobile worklist; delegate everything else."""
    ...
    ready_count = sum(1 for item in decorated if not (item.metadata or {}).get("studio_post_ids"))
    drafted_count = sum(1 for item in decorated if (item.metadata or {}).get("studio_post_ids"))
    draft_state = (request.GET.get("draft_state") or "ready").strip().lower()
    if draft_state not in {"ready", "drafted", "all"}:
        draft_state = "ready"

    if draft_state == "ready":
        decorated = [item for item in decorated if not (item.metadata or {}).get("studio_post_ids")]
